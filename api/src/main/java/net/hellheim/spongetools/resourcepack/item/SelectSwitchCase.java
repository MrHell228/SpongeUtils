...
public record SelectSwitchCase<T>(List<T> values, ItemModel model) {
	public static <T> Codec<SelectSwitchCase<T>> codec(Codec<T> codec) {
		return RecordCodecBuilder.create(instance -> instance.group(
				...
				ItemModel.CODEC.fieldOf("model").forGetter(SelectSwitchCase::model)
				).apply(instance, SelectSwitchCase::new));
	}
	
	public SelectSwitchCase(final List<T> values, final ItemModel model) {
